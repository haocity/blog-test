...
function creatul() {
  var arr = ["首页", "index.html", "日常",, "技术",, "软件",, "友链", "link.html", "应用中心", "https://app.haotown.cn"];
...
function loadpage(i, j, callback) {
  ...
  var url = city.api + 'page?s=' + i + '&e=' + j + ex;
...
function loadallone(pid) {
  fetch(city.api + 'one?id=' + pid).then(function (response) {
    ...
    city.footer.style.display = 'block';
    var warp = document.querySelector('.container ');
    var ec = creatpost(json.title, json.data, json.time, json.classify, json.commentsnumber, json.love, json.pid, "all", json.md);
    warp.appendChild(ec);
    opencom(ec);
  });
}
...
document.querySelector('.container').addEventListener('click', function (e) {
  ...
  if (ele.classList.contains("pointer")) {
    if (ele.classList.contains("post-bottom-comnum")) {
      ...
      //分享
      var url = city.url + "?pid=" + ele.parentNode.parentNode.pid;
      var title = "疯狂减肥带-" + ele.parentNode.parentNode.querySelector(".title>a").innerText;
